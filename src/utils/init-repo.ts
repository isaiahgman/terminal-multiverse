...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.error(
      chalk.red('Error: Current directory is not a Git repository. Run "git init" first.'),
    );
    await pause();
    process.exit(1);
  }
  ...
  try {
    ...
    console.error(
      chalk.red('Error: GitHub CLI ("gh") is not authenticated. Run "gh auth login" first.'),
    );
    await pause();
    process.exit(1);
  }
  ...
  const projName =
    (await prompt('Project Name (default "terminal-multiverse"): ')) || 'terminal-multiverse';
...
`;
  ...
  try {
    ...
    execSync('git commit -m "chore: initialize project context configurations"', {
      stdio: 'inherit',
    });
